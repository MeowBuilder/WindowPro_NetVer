...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	HWND hWnd = CreateWindow(L"WindowClass", L"타이틀 화면", WS_OVERLAPPEDWINDOW, (Desk_rect.right / 2) - 400, (Desk_rect.bottom / 2) - 300, 800, 600, nullptr, nullptr, hInstance, nullptr);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CREATE:
		...
		ssystem->createSound("bgm.wav", FMOD_LOOP_NORMAL, 0, &sound1); //--- 1번 사운드 생성 및 설정
		ssystem->createSound("button.wav", FMOD_LOOP_OFF, 0, &sound2); //--- 2번 사운드 생성 및 설정
		ssystem->createSound("jump.wav", FMOD_LOOP_OFF, 0, &sound3); //--- 3번 사운드 생성 및 설정
...
LRESULT CALLBACK WndProcGame(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CREATE:
		...
		ssystem->init(32, FMOD_INIT_NORMAL, extradriverdata); //--- 사운드 시스템 초기화
		ssystem->createSound("bgm.wav", FMOD_LOOP_NORMAL, 0, &sound1); //--- 1번 사운드 생성 및 설정
		ssystem->createSound("jump.wav", FMOD_LOOP_OFF, 0, &sound2); //--- 2번 사운드 생성 및 설정
		ssystem->createSound("down.wav", FMOD_LOOP_OFF, 0, &sound3); //--- 3번 사운드 생성 및 설정
	...
	case WM_CHAR:
		...
		case 'q': case 'Q':
			...
			ssystem->release();
			CloseGameWindow(hWnd); // 타이틀 화면으로 돌아감.
			break;
		default:
			break;
		}
...
void CALLBACK TimerProc(HWND hWnd, UINT uMsg, UINT idEvent, DWORD dwTime) {
	...
	case 1://플레이어 이동 및 충돌
		...
		{
			if (player.x <= map.blocks[i].Block_rt.right && player.x >= map.blocks[i].Block_rt.left)//플레이어가 특정블럭의 x안에 있다면
			{
				if (player.y + Size > map.blocks[i].Block_rt.top && player.y + Size < map.blocks[i].y)//위에
				{
				...
				else if (player.y - Size > map.blocks[i].y && player.y - Size < map.blocks[i].Block_rt.bottom)//아래
				{
			...
			else if (player.y <= map.blocks[i].Block_rt.bottom && player.y >= map.blocks[i].Block_rt.top)//플레이어가 특정블럭의 y안에 있다면
			{
				if (player.x - Size < map.blocks[i].Block_rt.right && player.x - Size > map.blocks[i].x)//오른쪽
				{
					player.UP = false;
				...
				else if (player.x + Size > map.blocks[i].Block_rt.left && player.x + Size < map.blocks[i].x)//왼쪽
				{
					player.UP = false;
		...
		if (map.boss_count != 0)
		{
			if (map.boss.attack_time <= 0)//일정 주기마다 블럭 생성
			{
			...
			for (int i = 0; i < map.block_count; i++)
			{
				...
				if (map.blocks[i].x <= Desk_rect.left)//블럭이 맵 밖으로 나가면
				{
...
LRESULT CALLBACK WndEditProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CHAR:
		...
		case 'q': case 'Q':
			...
			DeleteObject(BGM_bitmap);
			DeleteObject(BGN_bitmap);
			CloseEditWindow(hWnd); // 타이틀 화면으로 돌아감.
			break;
		default:
			break;
		}
